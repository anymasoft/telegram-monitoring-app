...
export function SectionCards({ data }: { data: MetricData[] }) {
  return (
        ...
        <Card key={index} className="@container/card">
          ...
          <CardFooter className="flex-col items-start gap-1.5 text-sm">
            <div className="line-clamp-1 flex gap-2 font-medium">
              {metric.trend === "up" ? "Рост" : "Снижение"} за период{" "}
              {metric.trend === "up" ? <IconTrendingUp className="size-4" /> : <IconTrendingDown className="size-4" />}
            </div>
